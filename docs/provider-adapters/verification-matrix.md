...
| Doubao | `doubao.com` | `DoubaoAdapter` | blocked | 2026-06-30: blocked. Browser: Chrome 149 with unpacked extension loaded via `Extensions.loadUnpacked`. Page: https://www.doubao.com/. Checks: 1-5 partial. Notes: content script injection, `DoubaoAdapter` activation, textarea composer insertion, and MCP popover visibility verified; submit selector drift found because committed submit selectors returned 0 matches after text insertion while the visible send button matched `button[class*="g-send-msg-btn"]`. Submit and result reinsertion were not verified; selector patch not applied because `content/index.iife.js` had unrelated pre-existing dirty changes. |
