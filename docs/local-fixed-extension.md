...
- 后端工具列表里可能出现 `read_media_file`，它通常只能把媒体文件读成 base64/MIME 这类传输格式；网页模型是否能理解图片内容，取决于当前网页模型和插件适配能力，暂不作为稳定能力。
- 当前旧插件不提供稳定的图片编辑、音频理解、视频处理或多模态文件写入流程。

简单说：现在先把“本地文本文件 + 目录操作”当成可靠能力，多模态能力后面需要单独设计和验证。
