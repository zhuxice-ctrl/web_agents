...
- 用户在任务中明确给出的路径会进入对应任务的可写范围；其他外部写入仍由授权流程控制。
...
文件系统安全底座使用独立版本 `@web-agents/local-core@1.0.1`，来源固定为仓库的发布标签。本分支不再包含圆桌代码、Core 源码副本或历史扩展副本。
...
读取操作可以访问用户明确提供的绝对路径。用户在任务中主动给出的写入路径会按任务意图授权；没有明确路径依据的写入、编辑、创建、移动和删除操作仍受权限控制并记录审计。
