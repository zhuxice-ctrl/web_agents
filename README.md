...
正式产品分支为 `tablellm`，`tablellm-v1` 保留为第一代发布线。插件产品位于独立的 `webagent` 分支，两个产品分支不互相合并。文件安全和权限能力依赖 `main` 发布的独立 `@web-agents/local-core` 版本。
...
- 模型生成中的流式内容放在可折叠区域内，不会强制改变当前阅读位置；本轮全部结束后可以使用向下箭头快速滚动到最新消息。
...
环境要求：Windows 10/11、Node.js 24 或更高版本，以及已登录所需模型网页的 Chrome 环境。
